...
class CountriesFilterer {
  ...
  /**
   * Prepares data for filtering. After that call [countries] and [countriesMetaInfo] should be stored
   * and calling [filter] will be possible
   */
  fun prepare(countries: List<Country>, countriesMetaInfo: Map<String, CountryMetaInfo>) {
    initValues(countries, countriesMetaInfo)
  }
  
  /**
   * Performs countries filtering. Do not forget to call [prepare] before calling this
   * function
   *
